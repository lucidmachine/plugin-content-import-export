...
public class ContentImporterThread implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        try {
            ...
            haveFileSource = UtilMethods.isSet(properties.get("haveFileSource"))
                            && (Boolean) properties.get("haveFileSource");
    ...
    private void importFromContent(String fileAsset, String fileAssetQuery, String logPath,
        ...
        try {
            ...
            List<Contentlet> hits = APILocator.getContentletAPI().search(luceneQuery, -1, 0,
                            "modDate asc", APILocator.getUserAPI().getSystemUser(), false);
            ...
            for (Contentlet contentlet : hits) {
                ...
                try {
                    ...
                    try {
                        ...
                        moveImportedAsset(contentlet,
                                        fileAssetCont, fileName + " - "
                                                        + UtilMethods.dateToHTMLDate(new Date(),
                                                                        "yyyyMMddHHmmss")
                                                        + ".csv.old");
                    ...
                    sendResults(results, reportEmail, fileName + " Import Results", logPath,
                                    fileName);
                }
            }
    ...
    // com.dotmarketing.portlets.fileassets.business.FileAssetAPIImpl.renameFile(Contentlet, String,
    // User, boolean)
    public void moveImportedAsset(Contentlet contentlet, FileAsset fileAssetCont, String newName)
        ...
        Host host = APILocator.getHostAPI().find(id.getHostId(),
                        APILocator.getUserAPI().getSystemUser(), false);
        Folder folder = APILocator.getFolderAPI().findFolderByPath(id.getParentPath(), host,
                        APILocator.getUserAPI().getSystemUser(), false);
        ...
        if (!APILocator.getFileAssetAPI().fileNameExists(host, folder, newName, id.getId())) {
            ...
            File newFile = new File(oldFile.getPath().substring(0,
                            oldFile.getPath().indexOf(oldFile.getName())) + newName);
            ...
            try {
                ...
                contentlet = APILocator.getContentletAPI().checkin(contentlet,
                                APILocator.getUserAPI().getSystemUser(), false);
                ...
                Logger.error(this, "Unable to rename file asset to " + newName + " for asset "
                                + id.getId(), e);
                throw e;
            } finally {
                if (newFile != null) {
    ...
    private void importFromFileSystem(String filePath, String logPath, String reportEmail,
                    String structure, String[] fields, long language, boolean isMultilanguage,
                    String csvSeparatorDelimiter, String csvTextDelimiter, boolean publishContent,
        ...
        for (File file : filesList) {
            if (!file.exists()) {
                ...
                ((List<String>) results.get("errors"))
                                .add("File: " + filePath + " can't be readed.");
                ...
                try {
                    ...
                    HashMap<String, List<String>> currentResults =
                                    importInputStream(new FileInputStream(file), csvTextDelimiter,
                                                    csvSeparatorDelimiter, structure, fields,
                                                    language, isMultilanguage, publishContent,
                                                    deleteAllContent, saveWithoutVersions);
                    if (currentResults != null) {
                        results = currentResults;
                    }
                } catch (Exception e) {
    ...
    private void moveImportedFile(String filePath) {
        try {
            String processedFilePath = pluginAPI.loadProperty("org.dotcms.plugins.contentImporter",
                            "processedFilePath");
            ...
            file.renameTo(new File(processedFilePathDir.getPath() + File.separator + fileNameNoExt
    ...
    private void sendResults(HashMap<String, List<String>> results, String reportEmail,
                    String subject, String logPath, String fileName) {
    ...
    private void contentImporterLogger(String filePath, String fileName, String message) {
        ...
        try {
            ...
            File outputFile = new File(folderPath.getPath() + File.separator + fileName + " - "
                            + UtilMethods.dateToHTMLDate(new Date(), "yyyyMMddHHmmss") + ".txt");
